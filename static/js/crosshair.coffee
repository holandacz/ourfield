...
# show a crosshair on map

if opts["zoom"] is "auto" or (not opts["zoom"] and opts["zoom"] isnt "0")
  opts["center"] = [ 40, -100 ]  if not opts["center"] or (opts["center"] and opts["center"].length is 0)
  map.setCenter new GLatLng(opts["center"][0], opts["center"][1]), 8, eval(opts["map_type"] or "G_NORMAL_MAP")
else
  map.setCenter new GLatLng(opts["center"][0], opts["center"][1]), opts["zoom"], eval(opts["map_type"] or "G_NORMAL_MAP")



if opts["center_coordinates"] isnt false
  unless $("gv_center_container")
    ...
    center_html += "<td><div id=\"gv_measurement_icon\" style=\"display:block; width:23px; height:15px; margin-left:3px; cursor:pointer;\"><a href=\"javascript:void(0);\" onclick=\"GV_Place_Measurement_Tools();\" title=\"Click here for measurement tools\" style=\"cursor:pointer;\"><img src=\"http://maps.gpsvisualizer.com/google_maps/ruler.gif\" width=\"23\" height=\"13\" border=\"0\" vspace=\"1\" class=\"gmnoprint\" /></a></div></td>"  if opts["measurement_tools"] isnt false
  ...
  if $("gv_crosshair")
    $("gv_crosshair").style.display = (if (opts["crosshair_hidden"]) then "none" else "block")
...
if opts["mouse_coordinates"]
  ...
  GEvent.addListener map, "mousemove", (mouse_coords) ->
    $("gv_mouse_coordinates").innerHTML = "Mouse: <span id=\"mouse_coordinate_pair\">" + parseFloat(mouse_coords.lat().toFixed(5)) + "," + parseFloat(mouse_coords.lng().toFixed(5)) + "</span>"  if $("gv_mouse_coordinates")

GV_Place_Measurement_Tools opts["measurement_tools"]  if opts["measurement_tools"] and opts["measurement_tools"]["visible"]