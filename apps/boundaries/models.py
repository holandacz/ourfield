...
# mysql --user=root --password= --execute="use _f; drop table of_boundaries; drop table of_boundaries_related_id;"
...
class Boundary(MyModel):
    ...
    class Meta:
        ...
        db_table = u'of_boundaries'        
        # db_table = u'of_boundaries_test'        
...
class BoundariesRelated(MyModel):
    ...
    class Meta:
        ...
        db_table = u'of_boundaries_related'   
...
class B(models.Model):
    ...
    def poly(self):
        ...
        cursor.execute("SELECT AsText(Points) FROM of_b WHERE id = %d LIMIT 1" % self.id)
        row = cursor.fetchone()
    ...
    def set_poly(self):
        ...
        cursor.execute("UPDATE of_b SET Points = GeomFromText( 'POLYGON((%s))' ) WHERE id = %d" % (self.pts, self.id))
